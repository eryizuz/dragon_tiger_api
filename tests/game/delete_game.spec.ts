...
test.group('Game: Delete game', (group) => {
  ...
  test('No token provider: Should return a 401 status code and an error message', async ({
    ...
    const response = await client.delete('/game/remove')
  ...
  test('Token not found: Should return a 400 status code and an error message', async ({
    ...
    const response = await client.delete('/game/remove').header('authentication-x-token', 'asd')

    response.assertStatus(400)
    response.assertBodyContains({ error: 'Token inválido!' })
  })

  test('Not provider Uuid: Should return a 401 status code and an error message', async ({
    client,
  }) => {
    ...
    response.assertStatus(401)
    response.assertBodyContains({ error: 'Debe enviar el uuid del juego!' })
  })
  ...
  test('Token and uuid OK: Should return a 200 status code and an success message', async ({
    ...
    const response = await client
      .delete(`/game/remove`)
      .header('authentication-x-token', user.body().authenticationToken)
      .json({ uuid: newGame.uuid })

    response.assertStatus(200)
    response.assertBodyContains({ message: 'Juego eliminado!' })
  })
})
