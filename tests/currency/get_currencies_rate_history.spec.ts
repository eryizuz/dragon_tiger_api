...
test.group('Currency: Get currencies rates history.', (group) => {
  ...
  test('No token provider: Should return a 401 status code and an error message', async ({
    ...
    const response = await client.get('/currency/rate-history')
  ...
  test('Token ok: Should return a 404 status code and an error message', async ({ client }) => {
    ...
    response.assertBodyContains({ message: 'Historial de tazas de cambio de las monedas listado!' })
  })
})
