...
test.group('Currency: Get all currencies.', (group) => {
  ...
  test('No token provider: Should return a 401 status code and an error message', async ({
    ...
    const response = await client.get('/currency/all')
  ...
  test('Token not found: Should return a 400 status code and an error message', async ({
    ...
    const response = await client.get('/currency/all').header('authentication-x-token', 'asd')

    response.assertStatus(400)
    response.assertBodyContains({ error: 'Token inválido!' })
  })

  test('Token ok: Should return a 404 status code and an error message', async ({ client }) => {
    ...
    response.assertBodyContains({ message: 'Monedas listadas!' })
  })
})
