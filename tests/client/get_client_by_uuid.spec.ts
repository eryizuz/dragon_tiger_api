...
test.group('Client: Get client by uuid.', (group) => {
  ...
  test('No token provider: Should return a 401 status code and an error message', async ({
    ...
    const response = await client.get('/client/get-one/asd')
  ...
  test('Token not found: Should return a 400 status code and an error message', async ({
    ...
    const response = await client.get('/client/get-one/asd').header('authentication-x-token', 'asd')

    response.assertStatus(400)
    response.assertBodyContains({ error: 'Token inválido!' })
  })

  test('Not token provider: Should return a 404 status code and an error message', async ({
    client,
  }) => {
    ...
    response.assertBodyContains({ message: 'E_ROUTE_NOT_FOUND: Cannot GET:/client/get-one' })
  })
  ...
  test('Token and uuid ok: Should return a 201 status code and an success message', async ({
    ...
    const user = await client.post('/auth/login').json({ userName: 'user', password: 'user' })
    const response = await client
      .get(`/client/get-one/${newClient.uuid}`)
      .header('authentication-x-token', user.body().authenticationToken)

    response.assertStatus(200)
    response.assertBodyContains({ message: 'Cliente listado!' })
  })
})
