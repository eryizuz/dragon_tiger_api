...
test.group('Exchange Api: Update exchange.', (group) => {
  ...
  test('No token provider: Should return a 401 status code and an error message', async ({
    ...
    const response = await client.put('/exchange/update/asd')
  ...
  test('Not data provided: Should return a 400 status code and an error message', async ({
    client,
  }) => {
    ...
    response.assertStatus(400)
    response.assertBodyContains({ error: 'Debe enviar el nombre o la url que sesea actualizar!' })
  })
  ...
  test('exchange uuid not found: Should return a 404 status code and an error message', async ({
    ...
    const response = await client
      .put('/exchange/update/asd')
      .header('authentication-x-token', user.body().authenticationToken)
      .json({ url: 'http://localhost:3000/api' })

    response.assertStatus(404)
    response.assertBodyContains({ error: 'No se encuentra el exchange!' })
  })

  test('Token, exchange uuid and data OK: Should return a 200 status code and an success message', async ({
    client,
  }) => {
    const user = await client.post('/auth/login').json({ userName: 'user', password: 'user' })
    const response = await client
      .put(`/exchange/update/${newExchange.uuid}`)
      .header('authentication-x-token', user.body().authenticationToken)
      .json({ url: 'http://localhost:3000/api' })

    response.assertStatus(200)
    response.assertBodyContains({ message: 'Exchange actualizado!' })
  })
})
